	...
	<script type="text/javascript">
		...
		while (it.hasNext()) {  
			...
			if(org.getParentcode()!=null&&!(org.getParentcode().equals(""))){
				System.out.println(org.getOrgcode()+":::"+org.getParentcode());
			%>
			d.add(<%=org.getOrgcode()%>,<%=org.getParentcode()%>,'<%=org.getOrgname()%>','','','allrubbishright');
			<%
			}else{
				System.out.println(org.getOrgcode()+":::"+org.getBelongcode());
				%>
				d.add(<%=org.getOrgcode()%>,<%=org.getBelongcode()%>,'<%=org.getOrgname()%>','allrubbishlist.jsp?orgcode=<%=org.getOrgcode()%>','','allrubbishright');
				<%
			}
		}
		%>
