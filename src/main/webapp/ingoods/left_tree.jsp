...
<body leftFrame="true">
	...
	<script type="text/javascript">
		...
		while (it.hasNext()) {  
			...
			%>
			d.add(<%=org.getOrgcode()%>,<%=org.getParentcode()%>,'<%=org.getOrgname()%>','','','allingoodsright');
			<%
			}else{
				System.out.println(org.getOrgcode()+":::"+org.getBelongcode());
				%>
				d.add(<%=org.getOrgcode()%>,<%=org.getBelongcode()%>,'<%=org.getOrgname()%>','listforsearch.jsp?orgcode=<%=org.getOrgcode()%>','','allingoodsright');
				<%
			}
		}
		%>
