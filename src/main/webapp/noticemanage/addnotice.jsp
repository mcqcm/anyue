...
<form name="form1" id="form1" action="notice!addNotice.action" target="noticemanage" method="post">
		...
		<tr>
			<td>标题：</td><td><input type="text" name="notice.tittle"/></td>
		</tr>
		<tr>
			<td>发布时间：</td><td><input type="text" class="Wdate" name="notice.publishtime" onfocus="new WdatePicker({lang:'zh-cn'})" value="<%=date%>" size="15"/></td>
		</tr>
		<tr>
			<td>内容：</td><td><textarea id="notice" name="content2" cols="20" rows="2" class="ckeditor"></textarea>
			<script type="text/javascript">
    ...
    CKEDITOR.replace('notice',
		...
		<tr>
			<td>邮件通告：</td>
			<td>
			<input type="checkbox" name="isEmail" value="all"/>发送给所有人
			<input type="checkbox" name="isEmail" value="shop"/>发送给餐饮店
			<input type="checkbox" name="isEmail" value="manager"/>发送给监管部门
			</td>
		</tr>
		<tr>
